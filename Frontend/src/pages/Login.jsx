...
import { Mail, Lock, ArrowRight, ArrowLeft, Sparkles, AlertCircle } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useTheme } from '../context/ThemeContext';

const getErrorMessage = (err) => {
  const code = err?.code || '';
  if (code === 'auth/email-not-verified') return 'Please verify your email before logging in. Check your inbox.';
...
const Login = () => {
  ...
  const [showResend, setShowResend] = useState(false); // ← show resend button if not verified
    ...
    <div className={T.pageBg}>
      ...
      >
          ...
          <div className={T.errorBox}>
            ...
            {showResend && (
              <button
                ...
                className={`mt-2 text-[10px] font-black uppercase tracking-widest underline ${isDark ? "text-amber-500" : "text-indigo-600"}`}
              >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input type="email" required className={T.input} placeholder="name@example.com"
          ...
          <button disabled={loading} className={T.submitButton}>
            {loading ? (
              <><div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />Authenticating...</>
            ) : (
        ...
        <div className="relative my-8">
          ...
          <div className={T.dividerText}>OR CONTINUE WITH</div>
        </div>
        ...
        <p className={T.footerText}>
          New here?
          <Link to="/register" className={T.footerLink}>Create Account</Link>
        </p>
      </motion.div>
    </div>
  );
